 ...
 * @see WP_List_Table
 */
class Admin_Live_Search_Posts_List_Table extends Admin_Live_Search_List_Table {
	...
	protected $admin_live_search_filter = 0;
	...
	public function __construct( $args = array() ) {
		...
		$this->admin_live_search_filter = intval($args['admin_live_search_filter']);
	...
	public function wp_edit_posts_query( $q = false ) {
		...
		// sorting
		$query['orderby'] = (isset($this->request['orderby'])) ? $this->request['orderby'] : '';
		$query['order'] = (isset($this->request['order'])) ? $this->request['order'] : '';

		// Hierarchical types require special args.
		if ( is_post_type_hierarchical( $post_type ) && empty($query['orderby']) ) {
		...
		if ( ! empty( $q['show_sticky'] ) )
			...
			// title only
			if ( $this->admin_live_search_filter == 1) {
				global $wpdb;
				$post_ids_title = $wpdb->get_col( $wpdb->prepare( "
			        SELECT DISTINCT ID FROM {$wpdb->posts}
							WHERE post_title LIKE '%s'", '%' . $wpdb->esc_like( $_REQUEST['s'] ) . '%' ) );
				if (empty($query['post__in'])) $query['post__in'] = array();
			  $query['post__in'] = array_merge( $query['post__in'] , $post_ids_title);
				unset($post_ids_title);
				$query['s'] = '';
			}
			// content only
			if ( $this->admin_live_search_filter == 2) {
				global $wpdb;
				$post_ids_content = $wpdb->get_col( $wpdb->prepare( "
			        SELECT DISTINCT ID FROM {$wpdb->posts}
							WHERE post_content LIKE '%s'", '%' . $wpdb->esc_like( $_REQUEST['s'] ) . '%' ) );
				if (empty($query['post__in'])) $query['post__in'] = array();
			  $query['post__in'] = array_merge( $query['post__in'] , $post_ids_content);
				unset($post_ids_content);
				$query['s'] = '';
			}

			// title and content(default)
			if ( $this->admin_live_search_filter == 0) {
				global $wpdb;
				$post_ids_content = $wpdb->get_col( $wpdb->prepare( "
			        SELECT DISTINCT ID FROM {$wpdb->posts}
							WHERE post_content LIKE '%s' OR post_title LIKE '%s'", '%' . $wpdb->esc_like( $_REQUEST['s'] ) . '%' , '%' . $wpdb->esc_like( $_REQUEST['s'] ) . '%' ) );
				if (empty($query['post__in'])) $query['post__in'] = array();
			  $query['post__in'] = array_merge( $query['post__in'] , $post_ids_content);
				unset($post_ids_content);
				$query['s'] = '';
			...
			// Fix when this query gives no results the array becomes empty
			if (empty($query['post__in'])) {
				$query['s'] = 'no_results_found_via_admin_live_search_on_this_query';
			}
...
<?php
	 ...
	 *
	 * @since 1.0.0 (Standalone function exists since 2.6.0)
